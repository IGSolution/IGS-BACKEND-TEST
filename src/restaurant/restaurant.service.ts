...
import { JwtPayload } from '../auth/strategies/jwt.strategy'; // Import the JwtPayload
...
export class RestaurantsService {
  ...
  async createRestaurant(createRestaurantDto: CreateRestaurantDto, user: JwtPayload): Promise<Restaurant> {
    const createdRestaurant = new this.restaurantModel({ ...createRestaurantDto, userId: user.userId });
    return createdRestaurant.save();
  }

  async findAllRestaurants(): Promise<Restaurant[]> {
    return this.restaurantModel.find().populate('menu', '_id name description price').exec();
  }
  ...
  async findRestaurantById(id: string): Promise<Restaurant> {
    ...
    const restaurant = await this.restaurantModel.findById(id).populate('menu', '_id name description price').exec();
    if (!restaurant) {
  ...
  async updateRestaurant(id: string, updateRestaurantDto: UpdateRestaurantDto, user: JwtPayload): Promise<Restaurant | null> {
    if (!Types.ObjectId.isValid(id)) {
      throw new NotFoundException(`Invalid restaurant ID: ${id}`);
    }
  ...
  async deleteRestaurant(id: string, user: JwtPayload): Promise<Restaurant | null> {
    if (!Types.ObjectId.isValid(id)) {
      throw new NotFoundException(`Invalid restaurant ID: ${id}`);
    }
    const restaurant = await this.restaurantModel.findById(id);
  ...
  async addMenuItem(restaurantId: string, createMenuItemDto: CreateMenuItemDto, user: JwtPayload): Promise<Restaurant> {
    ...
    const createdMenuItem = new this.menuItemModel({ ...createMenuItemDto, restaurant: restaurantId });
  ...
  async updateMenuItem(id: string, updateMenuItemDto: UpdateMenuItemDto, user: JwtPayload): Promise<MenuItem | null> {
    if (!Types.ObjectId.isValid(id)) {
      throw new NotFoundException(`Invalid menu item ID: ${id}`);
    }
  ...
  async deleteMenuItem(id: string, user: JwtPayload): Promise<MenuItem | null> {
    if (!Types.ObjectId.isValid(id)) {
      throw new NotFoundException(`Invalid menu item ID: ${id}`);
    }
    const menuItem = await this.findMenuItemById(id);
